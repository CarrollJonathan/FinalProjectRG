...
func (u *sessionsRepo) DeleteSession(token string) error {
	return u.db.Delete(&model.Session{}, "token = ?", token).Error // TODO: replace this
}

func (u *sessionsRepo) UpdateSessions(session model.Session) error {
	return u.db.Table("sessions").Where("email = ?", session.Email).Updates(session).Error // TODO: replace this
}
